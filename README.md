...
`PATH` или сделать `exit 0` и отрапортовать успех, ничего не выполнив. Всё, что не
является присваиванием или комментарием `#`, отвергается — как и попытка задать
`PATH`, `IFS`, `PYTHONPATH`, `LD_*`, `MAKEFLAGS` или зарезервированное имя образа.
Подстановки переменных внутри значений нет; один слой парных кавычек снимается.
