...
Checkpoint evidence, one directory per checkpoint — `artifacts/checkpoints/CP-00/`
holds the CP-00 manifest and acceptance records. It is not source, not a contract and
not an input to any build: it is how a checkpoint describes itself to someone who
clones the repository and asks what was accepted and against which commit.

`artifacts/local/` is gitignored scratch and is never committed.
