...
**Вывод, измеренный, а не выведенный: ни один freeze commit, взятый после `39a3a64`, не
может дать зелёную ратификацию.** Утверждение `W0-INT-03` «ратификационный коммит — первый
после заморозки, который может быть зелёным» ложно; его required test `discover -s
tests/contract` → exit 0 недостижим.
