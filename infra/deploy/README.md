...
| Path | What it is |
|---|---|
| `Dockerfile.api` | the eighteen operations under uvicorn, plus `T-3`'s health plane |
...
configured answers `authentication_required` to every one of the eighteen operations, while
...
```
:8000   the eighteen operations, mounted by the proxy at /api/v1
