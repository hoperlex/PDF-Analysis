...
| Path | What it is |
|---|---|
| `Dockerfile.api` | the seventeen operations under uvicorn, plus `T-3`'s health plane |
...
configured answers `authentication_required` to every one of the seventeen operations, while
...
```
:8000   the seventeen operations, mounted by the proxy at /api/v1
