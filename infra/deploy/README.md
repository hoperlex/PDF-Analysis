...
| Path | What it is |
|---|---|
| `Dockerfile.api` | the nineteen operations under uvicorn, plus `T-3`'s health plane |
...
configured answers `authentication_required` to every one of the nineteen operations, while
...
```
:8000   the nineteen operations, mounted by the proxy at /api/v1
