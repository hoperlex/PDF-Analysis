 ...
 *
 * Those figures are **sixteen operations across thirteen paths**, which is what the frozen
 * document declares after the wave-34 reseal that added `issueToken`. This file said
 * fifteen and twelve, which was true until that reseal and is the fifth stale count in
 * this programme, so it is no longer corrected by hand:
