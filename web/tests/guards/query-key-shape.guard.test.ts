...
describe('the compiler runs inside the gate, not only in a command nobody runs there', () => {
  ...
  it('is named in the Makefile, so the obligation cannot quietly leave the gate', () => {
     ...
     * `run_frontend`, where a gate can be read for what it checks -- and this case stays to
     * hold it there.
     *
     * Without it the move is reversible by deletion: someone tidies a Makefile line, the
     * suite stays green because it runs its own `tsc`, and the gate silently stops
     * typechecking again. That is `D-23`'s shape -- prose and behaviour drifting with
     * nothing comparing them -- pointed at the gate's own definition.
     */
