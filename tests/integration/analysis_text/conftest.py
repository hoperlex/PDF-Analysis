"""Fixtures for the integration suite, and the guard that keeps it offline.
...
``test_provider_modes.py`` then proves the guard actually fires, because a guard
nobody has watched fail is a guard that may have been silently disabled.
"""
