...
_DESCRIPTION: Final[str] = (
    "The twelve operations of the PC-01 surface. Every failure is one `ErrorEnvelope` "
    "carrying a catalog `error_code`, and every response carries `X-Correlation-Id`."
)

#: ``servers`` of the frozen document, verbatim. The twelve paths are declared relative to
...
def create_asgi_app(
    ...
    *,
    ...
    """The ASGI application: the twelve operations, the seam, and the four middlewares.
...
def create_documentation_app(environ: Mapping[str, str] | None = None) -> FastAPI:
    ...
    The served document is a function of the twelve declarations and the 43 models and not
...
def _drop_the_422_this_surface_cannot_answer(document: dict[str, Any]) -> dict[str, Any]:
    ...
    **Why this is not document surgery, and how you can tell.** `W13-CONF` measured that
    ...
    ``ValidationError`` out of ``components.schemas``. Eight of the twelve operations do
...
def _assemble(router: Router, environ: Mapping[str, str]) -> FastAPI:
    ...
    app = _ContractApplication(
        ...
        # `X-Output`. The 43 schema names are pinned by the contract and by the frontend's
