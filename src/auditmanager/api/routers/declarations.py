...
def envelope_responses(*statuses: int) -> dict[int | str, dict[str, Any]]:
    ...
    **Every operation that can refuse must declare its own 422 here.** `W13-CONF` measured
    ...
    (``test_the_gate_catches_fastapis_own_validation_error``). The 53 schema names are
...
#: that would be wrong. The contract: five of the nineteen operations declare no ``422`` at
